...
const TrackOrder = ({ navigation }) => {
    ...
    const getorders = async () => {
        const ordersRef = firebase.firestore().collection('UserOrders').where
            ('orderuseruid', '==', firebase.auth().currentUser.uid);
                ...
                {orders.sort(
                    ...
                    return (
                        <View style={styles.order} key={index}>
                            ...
                            <Text style={styles.total}>Total: Rs.{order.ordercost}</Text>
                            {
                                order.orderstatus === 'Delivered' ? <Text style=
                                    {styles.ordertxt3}>Thank you for ordering with us</Text> : null
                            }
                            {
                                order.orderstatus === 'cancelled' ? <Text style=
                                    {styles.ordertxt3}>Sorry for the inconvenience</Text> : null
                            }
                            {
